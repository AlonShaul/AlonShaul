...
const MagicGame = () => {
  ...
  // אתחול כוכבי לכת ייחודיים – 3 כוכבים שונים
  useEffect(() => {
    const newPlanets = [];
  ...
  // במצב מובייל – עדכון מיקום החללית לפי לחיצה + ירי קסם (ללא סאונד fire)
  useEffect(() => {
    const handleClick = (e) => {
  ...
  const draw = () => {
    ...
    // ציור אויבים – העיצוב נשאר בגווני אדום-צהוב בכל מצב
    enemiesRef.current.forEach(enemy => {
      ctx.save();
